...
public class Bar {
	 ...
	 */
	public static Bar getInstance(int cantidadMesas){
		if(bar == null){
	...
	public void cerrarMesa(int numeroMesa) throws NoDebeTenerConsumiciones, EstaCerrada{
		mesas[numeroMesa-1].cambiarACerrada();;
	}
	...
	public void agregarConsumicion(int numeroMesa,int codigo,int cantidad) throws DebeSerPositivo{
		...
		while(itr.hasNext() && !EncontroProducto){
			...
			System.out.println("Se a agregado la consumicion del Producto");
		}else{
	...
	public boolean eliminarProducto(int codigo){
		...
		while(itr.hasNext() && !fueEliminado){
			aux = itr.next();
			if(aux.getCodigo() == codigo){
	...
	public Estado estadoMesa(int numeroMesa){
	     return mesas[numeroMesa-1].consultarEstado();
	}				
